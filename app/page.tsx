...
function isPositionMode(mode: ScanMode) {
...
function pickColumns(activeColumns: string, deret: number[]) {
  return activeColumns.split("").map((c) => deret[COLS.indexOf(c)]).filter((n) => Number.isFinite(n));
}
...
function rowResultDigits(item: ScanItem, row: ScanRow) {
  ...
  return pickColumns(item.activeColumns, row.deret).map((digit) => ({ digit, hit: targets.includes(digit) }));
}
function rowResultText(item: ScanItem, row: ScanRow) { return rowResultDigits(item, row).map(({ digit }) => digit).join(""); }
...
function predictionResult(item: ScanItem) { return pickColumns(item.activeColumns, item.result.deretLive).join(""); }
...
export default function Page() {
  ...
  function bukaMarket() {
    ...
    setMarketQuery(""); setJenisOpen(false); setTargetOpen(false); setDigitOpen(false); setMarketOpen(true);
  }
  ...
  function pilihJenis(value: ScanMode) { setScanMode(value); setJenisOpen(false); }
    ...
    <div className="wrap">
      ...
      <div className="panel">
        ...
        <div className="row two">
          ...
          <div className="field digit-field"><label>{isOffMode(scanMode) ? "Jumlah OFF" : "Jumlah Digit"}</label><button className="digit-select" style={NO_BADGE_SELECT_STYLE} type="button" onClick={toggleDigit}><b>{digitCount} digit</b><span className="select-arrow">{digitOpen ? "⌃" : "⌄"}</span></button>{digitOpen && <div className="digit-menu">{DIGIT_OPTIONS.map((value) => <button key={value} type="button" style={NO_BADGE_OPTION_STYLE} className={value === digitCount ? "digit-option active" : "digit-option"} onClick={() => pilihDigit(value)}><span className="option-label">{value} digit</span>{value === digitCount && <b>✓</b>}</button>)}</div>}</div>
        </div>
      ...
      {result && <div className="panel result-panel"><p className="summary"><b>{marketName}</b> &middot; <b>{analysisTitle(result.config.scanMode, result.config.targetPos, result.config.target2D)}</b> &middot; {result.config.digitCount} digit &middot; {result.totalMatched} hasil</p><div className="scan-list compact-list">{result.items.length === 0 && <div className="scan-empty">Belum ada trek yang cocok.</div>}{result.items.map((item, index) => <div className="scan-item compact" key={`${item.scanMode}-${item.targetPos}-${item.target2D}-${item.formula}-${index}`}><span className="scan-formula compact-formula">{item.formula}</span><div className="compact-digits">{item.angkaHidup.map((digit, digitIndex) => <span key={`${digit}-${digitIndex}`}>{digit}</span>)}</div><button className="view-btn compact-view" type="button" onClick={() => { setCopied(false); setViewItem(item); }}>Lihat</button></div>)}</div>{result.items.length > 0 && <div className="frequency-block"><div className="frequency-head"><b>Frekuensi</b><span>{result.items.length} rumus</span></div><p>Kemunculan digit dari hasil scan.</p><div className="frequency-grid">{frequencies.map((item) => <div className={item.count === 0 ? "frequency-item muted" : "frequency-item"} key={item.digit}><b>{item.digit}</b><i>=</i><span>{item.count}x</span></div>)}</div></div>}</div>}

      {viewItem && <div className="sheet-bg" onClick={() => setViewItem(null)}><div className="sheet" onClick={(e) => e.stopPropagation()}><div className="sheet-head"><div><b>{detailTitle(viewItem)}</b><span>Rumus : {viewItem.formula.toLowerCase()}</span></div><div className="sheet-actions"><button className="copy-btn" type="button" onClick={copyTrek}>{copied ? "Tersalin" : "Salin Trek"}</button><button className="close-btn" type="button" onClick={() => setViewItem(null)}>×</button></div></div><div className="trek-detail">{viewRows.map((row, idx) => <div className="trek-row" key={`${row.displayDraw}-${idx}`}><span>{row.displayDraw}</span><i>➜</i><b className="row-digits">{rowResultDigits(viewItem, row).map(({ digit, hit }, digitIndex) => <span key={`${digit}-${digitIndex}`} className={hit ? "hit-digit" : ""}>{digit}</span>)}</b><em>{rowStatus(viewItem, row)}</em></div>)}<div className="trek-row pending"><span>{viewItem.result.latestDraw}</span><i>➜</i><b className="row-digits">{nextPrediction.split("").map((digit, index) => <span key={`${digit}-${index}`}>{digit}</span>)}</b><em>??</em></div></div></div></div>}

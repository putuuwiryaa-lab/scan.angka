  ...
  return (
    <main className="wrap admin-wrap">
      <header className="hero">
        ...
        <h1>Kelola Akses</h1>
        <p>Buat kode akses dan kelola device yang dapat menggunakan aplikasi.</p>
      </header>
      ...
      <section className="card panel admin-panel">
        ...
        <form className="access-form" onSubmit={generate}>
          ...
          />
          <button className="admin-button admin-button-primary" type="submit" disabled={pinLoading}>
            {pinLoading ? "Membuat Kode..." : "Buat Kode Akses"}
          </button>
        </form>
          ...
          {sessions.map((session) => (
            <div className="admin-row" key={session.id}>
              ...
              {session.revoked_at ? (
                ...
                <button className="admin-button admin-button-danger" type="button" onClick={() => revoke(session.id)}>
                  Hapus Akses Device
                </button>
              )}
            </div>
        ...
        <div className="admin-list">
          {pins.map((pin) => (
            <div className="admin-row" key={pin.id}>
              ...
              {pin.status === "unused" ? (
                <button className="admin-button admin-button-warning" type="button" onClick={() => revokePin(pin.id)}>
                  Batalkan Kode Ini
                </button>
              ) : null}
            </div>
      ...
      <button className="access-submit admin-logout-button" type="button" onClick={logout}>
        Keluar dari Admin
      </button>
    </main>
  );
